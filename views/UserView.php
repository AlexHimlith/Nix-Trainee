...
class UserView extends View implements UserViewInterface
{
    ...
    {
        $image = isset($user['img']) ? $user['img'] : 'icon-user.png';
        return "
            <form id='form_profile' method='post' action='/user/profile'>
                    <h3>{$_SESSION['nick']}</h3>
                    
                    <a href='/user/image'><img src='/views/img/$image'></a>
                    ...
                    <div>
                        ...
                        <label><span>Date of Birth:</span>
                            <input type='date' name='datebirth' id='datebirth' value='{$user['datebirth']}'>
                        </label>
